...
const Homepage = ({ cityCoordinates }) => {
  ...
  useEffect(() => {
    Promise.allSettled(
      cityCoordinates.map((city) => getLocalWeather(city.lat, city.lon))
    )
      .then((data) =>
        setCityDetails(
          data
  ...
  const findState = (lat, lon) => {
    let cityLocation = cityCoordinates.find(
      (city) =>
        city.lat.toFixed(2) === lat.toFixed(2) &&
        city.lon.toFixed(2) === lon.toFixed(2)
    );
    ...
    return weatherStatistics.map((city) => {
      return (
        <WeatherCard
          ...
          key={city.id}
          name={`${city.name}, ${
            findState(city.coord.lat, city.coord.lon) &&
            findState(city.coord.lat, city.coord.lon)
          }`}
          temp={city.main.temp}
    ...
    <section>
      <div className="card-display">
        {error ? (
          ...
          generateWeatherCards(cityDetails)
        )}
        {showModal && (
          <CityDetail selectedCity={selectedCity} handleClose={handleClose} />
        )}
      </div>
    </section>
  );
